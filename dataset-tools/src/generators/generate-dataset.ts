...
import { datasetConfig } from "../shared/dataset-config.js";
...
async function main(): Promise<void> {
  ...
  for (let companyIndex = 1; companyIndex <= datasetConfig.companies; companyIndex += 1) {
    ...
    for (let serviceIndex = 1; serviceIndex <= datasetConfig.servicesPerCompany; serviceIndex += 1) {
      ...
      for (let dayOffset = 0; dayOffset < datasetConfig.days; dayOffset += 1) {
        for (let slotIndex = 0; slotIndex < datasetConfig.slotsPerDay; slotIndex += 1) {
